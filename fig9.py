...
beta=[i/100 for i in range(25,50,10)]
def simu(beta):
    class Reaction:
        '''
    ...
    for i in range(n_jobs):
        ...
        paa = lambda t: lambda1 * alpha
        pab1 = lambda t: lambda1 * (1 - alpha)
        pab2 = lambda t: s1
        pbb = lambda t: lambda2 * (1 - beta)
        pba1 = lambda t: lambda2 * beta
        pba2 = lambda t: s2

        system.add_reaction(paa, [1, 0], [2, 0])
        system.add_reaction(pab1, [1, 0], [1, 1])
        system.add_reaction(pab2, [1, 0], [0, 1])
        system.add_reaction(pbb, [0, 1], [0, 2])
        system.add_reaction(pba1, [0, 1], [1, 1])
        system.add_reaction(pba2, [0, 1], [1, 0])
    ...
    np.savetxt(huizong_A_str + str(beta), huizong_A)
    np.savetxt(huizong_B_str + str(beta), huizong_B)
    np.savetxt(huizong_str + str(beta), huizong)
